...
if [ -L "$SYMLINK_PATH_KEYBOARD" ]; then
  print_header "Input Remapper desktop keyboard preset symlink ✅"
else
  # Create the symlink
  ln -s "$SOURCE_FILE_KEYBOARD" "$SYMLINK_PATH_KEYBOARD"
  print_header "Symlink created for Input Remapper desktop keyboard preset ✅"
fi
...
if [ -L "$SYMLINK_PATH_MOUSE" ]; then
  print_header "Input Remapper desktop mouse preset symlink ✅"
else
  # Create the symlink
  ln -s "$SOURCE_FILE_MOUSE" "$SYMLINK_PATH_MOUSE"
  print_header "Symlink created for Input Remapper desktop mouse preset ✅"
fi
